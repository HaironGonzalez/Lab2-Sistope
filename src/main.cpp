...
int main (int argc, char **argv)
{
  ...
  if (!config.fail() && Cflag){                // hay fichero y bandera
    cout << "Error Bandera C";
    exit (0);
  }
  else if (config.fail() && Cflag){                      // solo hay bandera 
    int pid;
    ...
    if (Cflag){
      ...
      for (int i=0;i<NumDiscos;i++){     // Se crean los hijos
  ...
  else if(!config.fail()){                               // solo hay fichero
    
    
    string palabra;
    config >> palabra;
    
    if (palabra.compare("Discos")==0){
      config >> palabra;
      
      
      NumDiscos = atoi(palabra.c_str());
      cout << " la palabra es "<< NumDiscos; 
    }
    else {
      cout << "Error en fichero Config"<<endl;
      exit(0);
    }
